...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	/* check if this pid is being monitored
	 change the status if it is, otherwise do nothing */
	if (table[reg.ax].monitored == 1) {
		status = check_pid_monitored(reg.ax, current->pid);
	}

	// check if all pid are being monitored
	else if (table[reg.ax].monitored == 2) {
		status = 1;

	}

	// do nothing if pid is not monitored

	spin_unlock(&my_table_lock);

	// log the message if status is 1, otherwise do nothing
	if (status == 1) {
...
/* help function for command request_syscall_intercept
*/
static int request_syscall_intercept(int syscall) {

	/* check permission for root
	*/
	if (current_uid() != 0) {
		return -EPERM;	
	}
	...
	/* check if the system call is already being intercepted
	*/
	...
	*/
	...
	spin_lock(&my_table_lock);
	// change the intercepted status for this system call
...
/* help function for command request_syscall_release
*/
static int request_syscall_release(int syscall) {
	/* check permission for root
	*/
	if (current_uid() != 0) {
		return -EPERM;	
	...
	/* check if the system call is being intercepted
	*/
	spin_lock(&my_table_lock);
	...
	/* release intercepted system call
	*/
	spin_lock(&sys_call_table_lock);
	set_addr_rw((unsigned long)sys_call_table);
	...
	// Reset monitored status
	table[syscall].monitored = 0; 
	spin_unlock(&sys_call_table_lock);
	spin_lock(&my_table_lock);
	destroy_list(syscall);
	// Reset intercpted status
...
/* help function for command request_start_monitoring
*/
static int request_start_monitoring(int syscall, int pid) {
	spin_lock(&my_table_lock);
	// check if the system call is being intercepted or not
	if (table[syscall].intercepted == 0) {
		spin_unlock(&my_table_lock);
		return -EINVAL;
	} else {
	...
	*/
	if (pid == 0) {
		...
		// case where all system calls are being monitored
		spin_lock(&sys_call_table_lock);
	...
	/* Two cases where pid is not valid:
	   1) negative pid
	   2) pid is not belong to another valid process
	*/
	else if (pid < 0){
		return -EINVAL;	
	} 
	...
	*/
		...
		if (table[syscall].monitored == 2) {
			...
			spin_unlock(&sys_call_table_lock);
			spin_lock(&my_table_lock);
			// target system call is already being monitored
			if (check_pid_monitored(syscall, pid) == 1) {
				spin_unlock(&my_table_lock);
				return -EBUSY;	

			} else {
				// fail to add a pid to a syscall's list of monitored pids.
...
/* help function for command request_stop_monitoring
*/
static int request_stop_monitoring(int syscall, int pid) {
	spin_lock(&my_table_lock);
	// check if the system call is being intercepted or not
	if (table[syscall].intercepted == 0) {
		spin_unlock(&my_table_lock);
	...
	*/
	if (pid == 0) {
		...
		// stop monitoring and destory the list
		spin_unlock(&sys_call_table_lock);
		spin_lock(&my_table_lock);
		destroy_list(syscall);
		spin_unlock(&my_table_lock);
	...
	*/
		...
		// system call is not being monitored
		if (table[syscall].monitored == 0) {

			spin_unlock(&sys_call_table_lock);
			return -EINVAL;	

		// some system call are being monitored
		} else if (table[syscall].monitored == 1) {
			spin_unlock(&sys_call_table_lock);
			spin_lock(&my_table_lock);
			// target syscall is not being monitored
		...
		// case where stop monitoring request comes for a specfic PID for a syscall monitors all pids
		} else {
			return -EINVAL;
		}
	}
 ...
 * 
 * - Make sure to use synchronization to ensure consstency of shared data structures.
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	// case where cmd is REQUEST_SYSCALL_INTERCEPT 
	if (cmd == REQUEST_SYSCALL_INTERCEPT) {

		int result = request_syscall_intercept(syscall);
		return result;

	} 

	// case where cmd is REQUEST_SYSCALL_RELEASE
	else if (cmd == REQUEST_SYSCALL_RELEASE) {

		int result = request_syscall_release(syscall);
		return result;

	} 

	// case where cmd is REQUEST_START_MONITORING 
	else if (cmd == REQUEST_START_MONITORING) {
		int result = request_start_monitoring(syscall, pid);
		return result;
	} 

	// case where cmd is REQUEST_STOP_MONITORING
	else if (cmd == REQUEST_STOP_MONITORING) {
		int result = request_stop_monitoring(syscall, pid);
		return result;
	} 
...
static int init_function(void) {
	...
	//saving original MY_CUSTOM_SYSCALL and exit_group system call
