...
int allocate_frame(pgtbl_entry_t *p) {
	int i;
	int frame = -1;
	// find the first page frame in memory that is not in used
	for(i = 0; i < memsize; i++) {
		if(!coremap[i].in_use) {
			frame = i;
			break;
	...
	if(frame == -1) { // Didn't find a free page.
		...
		// get the physical page frame of the victim in physmem
		pgtbl_entry_t* evicted_page = coremap[frame].pte;
		int is_modified = evicted_page->frame & PG_DIRTY;
		int is_onswap = evicted_page->frame & PG_ONSWAP;

		// if the victim page is modified or not on the swap file, then it need to write to the swap space
		if((!is_onswap) || is_modified){
			int offset;
			// write the victim page to the swap file
			if ((offset = swap_pageout(frame, (int)evicted_page->swap_off)) == INVALID_SWAP){
				perror("write to swap failed");
				exit(1);
			}
			// update the location offset in the swap file for next time usage
			evicted_page->swap_off = (off_t)offset;
			// indicates that no longer exists in the physical memory, is clean, and on swap file
			evicted_page->frame = (evicted_page->frame & ~PG_VALID & ~PG_DIRTY) | PG_ONSWAP;
		}
		if(is_modified){
			evict_dirty_count++;
		}else{
			// the page on swap file and not modified(clean)
			evict_clean_count++;
		}
	}
...
char *find_physpage(addr_t vaddr, char type) {
	...
	// 2nd page table is invalid at the directory entry
	if((pgdir[idx].pde & PG_VALID) == 0){
		// initialize a new second level page table and update the directory entry
	...
	// when the page is not in physical memory aka: not valid
	if(!(p->frame & PG_VALID)){
		int sim_frame = allocate_frame(p);  // allocate a page frame for the pte
		
		int new_entry = sim_frame << PAGE_SHIFT;
		// subcase1 : page is on the swap file
		if(p->frame & PG_ONSWAP){
			// fill the page data from swap file **into** physical memory
			if(swap_pagein(sim_frame, p->swap_off) != 0){
				perror("failed to fill data from the swap file");
				exit(1);
			}
			// reset the present bit in PTE to indicates the page is in physical memory
			new_entry = new_entry & ~PG_ONSWAP;
		}else{  // subcase2 : page is not on the swap file
			init_frame(sim_frame, vaddr);
			new_entry = new_entry | PG_DIRTY;	
		}
		// update the pageframe
		p->frame = new_entry;		
		// page not exist in physical memory
		miss_count++;
	}else{
		// page exist in physical memory
		hit_count++;
	}

	// at this point, the virtual page is in the physical memory

	// mark p as referenced and valid for ALL the cases
